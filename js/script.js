...
function setupTestimonialsSection() {
    $('.comment').text($('#author3').attr('data-comment'));
    $('.author').text($('#author3').attr('data-author'));

    $('.img-circle-border').click(function(){

        $('.img-circle-border').removeClass('active');
        
        $('.comment').text($(this).attr('data-comment'));
        $('.author').text($(this).attr('data-author'));
...
function initialize() {
    var mapOptions = {
      zoom: 15,
      center: new google.maps.LatLng(-34.583298, -58.418763)
    };
...
function setupContactSection() {
    ...
    $('#map-canvas').height($('.contact-form').height());    
}
